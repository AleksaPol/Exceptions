public class hw1 {
  ...
  static void DifferencArray() {
    try {
      int[] res = getDiffArray(new int[] { 8, -2, 6, 0, -7 }, new int[] { 2, 9, -6, 16, 5 });
  ...
  static int[] getDiffArray(int[] arr1, int[] arr2) {
    if (arr1 == null || arr2 == null) {
      ...
      // NullPointerException - это исключения, возникающие при попытке использовать
      // ссылку,
...
/**
 * Свой класс обработки ошибок связанных с размером массива наследуемый от
 * RuntimeException
 */
class CustomArraySizeException extends RuntimeException {
  ...
  /**
   * Конструктор для расширения конструктора по умолчанию, для того чтоб передать
   * в исключении
